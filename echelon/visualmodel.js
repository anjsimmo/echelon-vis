...
// flatMap not available in node v10.16.3 (although is in latest version).
// Shim to support it here.
// https://stackoverflow.com/questions/39837678/why-no-array-prototype-flatmap-in-javascript/39838385#39838385
const concat = (x,y) =>
  x.concat(y);
const flatMap = (f,xs) =>
...
function recommend(dataSchema, visGrammar) {
...
function bindprop(notation, data) {
  if (typeof(notation) === "object") {
    ...
    if ("bind_num" in notation) {
      ...
      let outMin = 1.0; //notation["min_o"]; // IGNORE: (always share full range)
      let inMax = notation["max_i"];
      let outMax = 15.0; // notation["max_o"];
...
function bind(notationFull, data, nodename = "graph") {
...
function createSymbolSvgFull (posx, posy, size, value, texture, color, orient, shape) {
...
function createSymbolSvg (color, shape) {
